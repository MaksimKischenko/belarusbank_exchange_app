      ...
      child: BlocConsumer<ExchangeBloc, ExchangeInfoState>(
        ...
        builder: (context, state) {
          ...
          if (state is ExchangeInfoLoading) {
            ...
            var city = state.infoTabs.isEmpty
                ? ''
                : state.infoTabs.first.name ?? 'Неизвестно';
            items.addAll(state.infoTabs.toList());
            items.sort((a, b) => double.tryParse(b.eurIn!)!
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return Dismissible(
                                    ...
                                    child: Card(
                                      color: double.tryParse(item.eurIn!)! ==
                                              sortPositionMaxEur(items, index)
                                          ? AppStyles.mainColor
                                          : Colors.white,
                                        ...
                                        children: [
                                          ListTile(
                                            ...
                                            title: SelectableText(
                                                '${item.streetType} ${item.street} ${item.homeNumber}',
                                                style: TextStyle(
                                                    color: double.tryParse(
                                                                item.eurIn!)! !=
                                                            sortPositionMaxEur(
                                            ...
                                            subtitle: currencies
                                                    .contains('Доллар США')
                                                ? SelectableText(
                                                    'Покупка: ${item.usdIn} - Продажа: ${item.usdOut}',
                                                    style: TextStyle(
                                                        color: double.tryParse(item.eurIn!)! !=
                                                                sortPositionMaxEur(
                                                                    items, index)
                                                            ...
                                                            : Colors.white))
                                                : SelectableText(
                                                    'Покупка: ${item.eurIn} - Продажа: ${item.eurOut}',
                                                    style: TextStyle(
                                                        color: double.tryParse(item.eurIn!)! !=
                                                                sortPositionMaxEur(
                                                                    items, index)
